...
async def async_setup(core: ApplicationCore, config: dict) -> bool:
    ...
    try:
        ...
        check_or_download(
            filename=filename_sunattributes, download_url=synset_url_sunattributes
        )
        check_or_download(
            filename=filename_wideresnet, download_url=synset_url_wideresnet
        )
...
async def async_process_images(
    core: ApplicationCore, config: dict, images: Dict
) -> bool:
