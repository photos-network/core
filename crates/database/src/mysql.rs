...
impl Database for MySQLDatabase {
    ...
    async fn get_account_by_id(&self, account_id: &str) -> Result<Account> {
        let account = sqlx::query_as::<_, Account>(
            "SELECT account_id, email, password_hash, display_name, created_at, updated_at, last_login_at, is_admin FROM accounts WHERE account_id = ?"
        )
        .bind(account_id)
        .fetch_optional(&self.pool)
        .await?;
    ...
    async fn link_access_code_to_account(&self, account_id: &str, customer_id: &str) -> Result<()> {
        sqlx::query(
            "INSERT INTO account_access_codes (account_id, customer_id) VALUES ($1, $2) ON CONFLICT DO NOTHING"
        )
        .bind(account_id)
        .bind(customer_id)
        .execute(&self.pool)
    ...
    async fn create_album(&self, owner_id: &str, name: &str, description: Option<&str>) -> Result<String> {
        ...
        sqlx::query(
            "INSERT IGNORE INTO album_accounts (account_id, album_id, role) VALUES ($1, $2, 'owner')"
        )
        .bind(owner_id)
        .bind(&album_id)
        .execute(&self.pool)
    ...
    async fn add_media_to_album(&self, album_id: &str, media_id: &str) -> Result<()> {
        sqlx::query(
            "INSERT INTO album_media (album_id, media_id) VALUES ($1, $2) ON CONFLICT DO NOTHING"
        )
        .bind(album_id)
        .bind(media_id)
        .execute(&self.pool)
    ...
    async fn get_media_for_album(&self, album_id: &str) -> Result<Vec<MediaItem>> {
        let rows = sqlx::query(
            "SELECT m.uuid, m.name, m.added_at, m.taken_at FROM media m \
             ...
             WHERE am.album_id = $1 ORDER BY am.position ASC, am.added_at ASC"
        )
        .bind(album_id)
        .fetch_all(&self.pool)
        .await?;
    ...
    async fn assign_album_to_customer(&self, album_id: &str, customer_id: &str) -> Result<()> {
        sqlx::query(
            "INSERT INTO customer_albums (customer_id, album_id) VALUES ($1, $2) ON CONFLICT DO NOTHING"
        )
        .bind(customer_id)
        .bind(album_id)
        .execute(&self.pool)
    ...
    async fn grant_album_to_account(
        ...
        sqlx::query(
            "INSERT INTO album_accounts (account_id, album_id, role) VALUES ($1, $2, $3) ON CONFLICT (account_id, album_id) DO UPDATE SET role = EXCLUDED.role"
        )
        .bind(account_id)
        .bind(album_id)
        .bind(role)
